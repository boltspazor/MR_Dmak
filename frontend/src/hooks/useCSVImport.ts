...
export const useCSVImport = ({ contacts, groups, onSuccess }: UseCSVImportProps) => {
  ...
  const handleCSVImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setUploadMessage(`Starting upload of ${totalRows} MRs...`);
      setUploadErrors([]);
      setShowUploadProgress(true);
      ...
      for (let rowIndex = 0; rowIndex < dataRows.length; rowIndex++) {
        ...
        const line = dataRows[rowIndex].trim();
        
        setUploadMessage(`Processing record ${rowIndex + 1} of ${totalRows}...`);
        ...
        if (totalSuccessful > 0 && (rowIndex + 1) % 10 === 0) {
          setUploadMessage(`Refreshing data after processing ${rowIndex + 1} records...`);
          onSuccess();
        }
      }
