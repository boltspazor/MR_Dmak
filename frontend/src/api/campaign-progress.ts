...
export const campaignProgressAPI = {
  ...
  getAllCampaignsProgress: async (params?: {
    ...
    const response = await api.get('/campaign-progress', { params });
    return response.data.data;
  },
  ...
  getMessageStatus: async (messageId: string): Promise<MessageStatus> => {
    const response = await api.get(`/campaign-progress/message/${messageId}/status`);
    return response.data.data;
  },
  ...
  updateMessageStatus: async (data: {
    ...
    const response = await api.post('/campaign-progress/webhook/status', data);
    return response.data.data;
  }
};
